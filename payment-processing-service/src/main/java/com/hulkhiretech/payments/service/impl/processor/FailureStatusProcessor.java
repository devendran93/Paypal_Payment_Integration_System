...
/**
 * Processor for the "Failure" transaction status.
 * No business logic is implemented; method logs and returns a string.
 */

@Service
@Slf4j
public class FailureStatusProcessor extends AbstractTxnStatusProcessor {

	public FailureStatusProcessor(
    ...
    public TransactionDTO processInternal(TransactionDTO transactionDTO) {
        ...
        log.info("FailureStatusProcessor.process called with transactionDTO: {}", transactionDTO);
        
                
        TransactionEntity entity = modelMapper.map(transactionDTO, TransactionEntity.class);
        log.info("FailureStatusProcessor converted TransactionDTO to TransactionEntity: {}", transactionDTO);
        
        boolean isUpdated = transactionRepository.updateTransactionDetailsById(entity);
        log.info("FailureStatusProcessor called updateTransactionDetailsById on repository with entity: {} and txnStatus: {}", 
        		entity.getId(), entity.getTxnStatusId());
        
        if(!isUpdated) {
			log.warn("FailureStatusProcessor failed to update TransactionEntity in repository: {}", entity.getId());
        ...
        log.info("FailureStatusProcessor successfully updated TransactionEntity in repository: {}", entity.getId());
